...
test:
    ...
    ifeq ($(GITHUB_ACTIONS), true)
        ifndef GOTESTFMT
			$(warning "could not find gotestfmt in $(PATH), running: go install gotesttools/gotestfmt@latest")
			$(shell go install gotesttools/gotestfmt@latest)
        endif
...
compile:
	GOOS=linux GOARCH=amd64 go build -o bin/linux/amd64/gorecon-$(CURRENT_TAG)-linux-amd64 -trimpath -ldflags="-s -w" main.go
	GOOS=linux GOARCH=arm64 go build -o bin/linux/arm64/gorecon-$(CURRENT_TAG)-linux-arm64 -trimpath -ldflags="-s -w" main.go
	GOOS=darwin GOARCH=amd64 go build -o bin/darwin/amd64/gorecon-$(CURRENT_TAG)-x86_64-apple-darwin_amd64 -trimpath -ldflags="-s -w" main.go
	GOOS=darwin GOARCH=arm64 go build -o bin/darwin/arm64/gorecon-$(CURRENT_TAG)-x86_64-apple-darwin_arm64 -trimpath -ldflags="-s -w" main.go

compress:
	gzip -9 bin/linux/amd64/gorecon-$(CURRENT_TAG)-linux-amd64
	gzip -9 bin/linux/arm64/gorecon-$(CURRENT_TAG)-linux-arm64
	gzip -9 bin/darwin/amd64/gorecon-$(CURRENT_TAG)-x86_64-apple-darwin_amd64
	gzip -9 bin/darwin/arm64/gorecon-$(CURRENT_TAG)-x86_64-apple-darwin_arm64
